...
angular.module( "ngAutocomplete", [])
  .directive('ngAutocomplete', function($parse , $http) {
    return {
          ...
          google.maps.event.addListener(scope.gPlace, 'place_changed', function() {
            scope.$apply(function() {
                ...
                $('#lat').val(lat).trigger('input');
                 $('#formatted_address').val(formatted_address).trigger('input');
                
                
                 
                if(place.address_components[3]){
                    var zipcode = (place.address_components[3]['long_name']);
                    $('#postcode').val(zipcode).attr("disabled", "disabled");
